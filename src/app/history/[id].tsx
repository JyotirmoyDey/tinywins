...
const HistoryRow = memo(function HistoryRow({ taskId, date, today, onEdit }: { taskId: string; date: string; today: string; onEdit: (date: string) => void }) {
  const entry = useEntry(taskId, date);
  return <Pressable accessibilityRole="button" accessibilityLabel={`${dayLabel(date, today)}, ${entry?.optionLabelAtEntry ?? 'No entry'}, edit response`}
...
function DayEditor({ task, date, onClose }: { task: Task; date: string; onClose: () => void }) {
    ...
    <View style={styles.overlay}><Pressable style={StyleSheet.absoluteFill} onPress={onClose} accessibilityRole="button" accessibilityLabel="Close response editor" />
      <View style={[styles.sheet, { paddingBottom: Math.max(insets.bottom, s.lg) }]} accessibilityViewIsModal>
        <ScrollView keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.choices}>{task.options.map(option => <RatingOptionButton key={option.id} label={option.label} disabled={false}
  ...
  return <SafeAreaView style={styles.screen}>
    ...
    {error ? <View style={{ padding: s.xxl, gap: s.lg }}><Text style={t.body}>History could not be loaded.</Text><Button label="Try again" onPress={() => { load(); }} /></View>
      ...
      : <FlatList style={{ marginHorizontal: s.xxl, backgroundColor: c.surface, borderRadius: r.lg, borderWidth: 1, borderColor: c.border }} data={dates} keyExtractor={date => date} contentContainerStyle={{ paddingHorizontal: s.md, paddingBottom: s.md }}
        renderItem={({ item }) => <HistoryRow taskId={id} date={item} today={today} onEdit={setEditing} />}
    ...
    {editing && task && <DayEditor key={`${id}:${editing}`} task={task} date={editing} onClose={() => setEditing(null)} />}
  </SafeAreaView>;
}
