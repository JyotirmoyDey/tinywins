...
import { CustomRange, getInsightsPeriod } from '../../analytics/insightsDateRange';
...
function monthCells(taskId:string|undefined, entries:AnalyticsEntry[], month:string, color:string){ const period=monthPeriod(month); const days=Number(period.endDate.slice(-2)); return Array.from({length:days},(_,i)=>{const date=`${month}-${String(i+1).padStart(2,'0')}`;const entry=entries.find(item=>item.taskId===taskId&&item.localDate===date);return {date,ratio:entry?Math.max(0.08,entry.normalizedWeightAtEntry/100):0,label:entry?.optionLabelAtEntry,color};}); }
...
export default function Insights(){
  ...
  const calendarCells=useMemo(()=>isAll?allMonthCells(tasks,dataset.entries,month):monthCells(selectedTask?.id,dataset.entries,month,selectedTask?.color||c.selected),[isAll,tasks,selectedTask,dataset.entries,month]);
  ...
  const individualCharts=selectedTask?[
    {id:ratingTrendConfig.id,order:ratingTrendConfig.displayOrder,visible:ratingTrendConfig.visible,
      element:<AnalyticsCard key={ratingTrendConfig.id} title={ratingTrendConfig.title} description={trendView?.presentation==='weekly'?'Weekly overview':'Your ratings over time.'}>{trend&&trendView&&<RatingTrendChart key={`${source}-${selectedTask.id}-${period.startDate}-${period.endDate}`} data={trend} view={trendView} taskName={selectedTask.name}/>}</AnalyticsCard>},
    ...
    {id:activityCalendarConfig.id,order:activityCalendarConfig.displayOrder,visible:activityCalendarConfig.visible,
      element:<AnalyticsCard key={activityCalendarConfig.id} title={activityCalendarConfig.title}><MonthCalendar month={month} onMonthChange={setMonth} cells={calendarCells} color={selectedTask.color} kind="individual"/></AnalyticsCard>},
  ...
  return <SafeAreaView style={styles.screen} edges={['top']}>
    ...
    {customOpen&&<NativeDateRangePicker applied={custom} today={today} onCancel={()=>setCustomOpen(false)} onApply={range=>{setCustom(range);setTimeline('CUSTOM');setCustomOpen(false);}}/>}
  </SafeAreaView>;
}
