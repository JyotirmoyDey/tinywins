...
function DockIcon({ action, active }: { action: DockAction; active: boolean }) {
  const color = action === 'add' ? c.selectedText : active ? c.selected : inactive;
...
export function BottomDock({ state, navigation, insets }: DockProps) {
  ...
  const onPress = (action: DockAction) => {
    ...
    else if (action === 'insights') navigation.navigate('insights', { task: 'all' });
    ...
    <View style={styles.items}>
      {(['index', 'insights', 'add', 'profile'] as const).map(action => {
        ...
        return <Pressable key={action} accessibilityRole={action === 'add' ? 'button' : 'tab'} accessibilityLabel={action === 'add' ? 'Add task' : label}
          accessibilityState={action === 'add' ? undefined : { selected: active }} onPress={() => onPress(action)}
          style={({ pressed }) => [styles.item, pressed && styles.pressed]}>
          <View style={action === 'add' ? styles.addCircle : styles.icon}><DockIcon action={action} active={active} /></View>
