...
export function MultiLineChart({series}:{series:{color:string;values:(number|null)[]}[]}){const w=Math.max(260,useWindowDimensions().width-64),h=180;return <Svg width={w} height={h} accessibilityLabel="Weekly comparison chart">{series.map((line,j)=>{const pts=coords(line.values,w,h);let d='';let open=false;pts.forEach(p=>{if(!p){open=false;return;}d+=(open?' L ':' M ')+`${p.x} ${p.y}`;open=true;});return <Path key={j} d={d} fill="none" stroke={line.color} strokeWidth={2}/>})}</Svg>}
