...
export function MultiLineChart({series}:{series:{color:string;values:(number|null)[]}[]}){const w=Math.max(260,useWindowDimensions().width-64),h=180;return <Svg width={w} height={h} accessibilityLabel="Multi activity weekly chart">{series.map((line,j)=>{const pts=coords(line.values,w,h);let d='';let open=false;pts.forEach(p=>{if(!p){open=false;return;}d+=(open?' L ':' M ')+`${p.x} ${p.y}`;open=true;});return <Path key={j} d={d} fill="none" stroke={line.color} strokeWidth={2}/>})}</Svg>}
export function BarChart({items,color}:{items:{label:string;value:number|null;count?:number|string;percentage?:number}[];color?:string}){const max=Math.max(1,...items.map(x=>x.value||0));return <View style={styles.bars}>{items.map((x,i)=><View key={i} style={styles.barRow}><Text style={styles.barLabel}>{x.label}</Text><View style={styles.barTrack}><View style={[styles.barFill,{width:`${((x.value||0)/max)*100}%`,backgroundColor:color||'#1F1F1D'}]}/></View><Text style={styles.barValue}>{x.count??(x.value===null?'—':Math.round(x.value))}</Text>{x.percentage!==undefined&&<Text style={styles.barPercent}>{Math.round(x.percentage)}%</Text>}</View>)}</View>}
